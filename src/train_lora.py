...
def load_config(path: str) -> LoraTraining:
    ...
    try:
        ...
        log.error(f"Error while loading full LORA training config: {e!s}")
        raise Exception(e) from e

def parse_args():
    p = argparse.ArgumentParser()
    p.add_argument("--config", required=True, help="Path to configs/lora.yaml")
    return p.parse_args()

def build_model_with_lora(cfg: LoraTraining):
